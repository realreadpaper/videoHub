...
默认按 **FastVideo 的 OpenAI 兼容 server**（`fastvideo serve`）实现：

| Hypit 生命周期 | 实际调用的接口 |
| --- | --- |
| `start` | `POST {baseUrl}/v1/videos` → `{ id, status, progress, ... }` |
| `poll` | `GET {baseUrl}/v1/videos/{id}` → `{ status: queued \| in_progress \| completed \| failed }` |
| `collect` | `GET {baseUrl}/v1/videos/{id}/content` → `video/mp4` 字节 |

换成别的服务时，改三个 **★ 服务适配点**（都在 `src/provider.ts` 里标出来了）：

| 适配点 | 位置 | 换服务时要做什么 |
| --- | --- | --- |
| 1 · 素材托管 | `start` 的 `compileWireRequest` 回调 | FastVideo 的 `image_reference` 只收 URL，需把字节放到服务器能拉取的位置。纯文生视频不走这里 |
| 2 · 任务提交 | `start` 末尾 | 改成你服务的提交接口，响应里要有任务 id |
| 3 · 状态与产物 | `poll` / `collect` | 改成你服务的状态查询与产物下载路径 |

其他协议形状的对应关系：

| 你的服务 | 提交 | 轮询 | 取产物 |
| --- | --- | --- | --- |
| FastVideo（本骨架） | `POST /v1/videos` | `GET /v1/videos/{id}` | `GET /v1/videos/{id}/content` |
| ComfyUI | `POST /prompt` | `GET /history/{prompt_id}` | `GET /view?filename=` |
| 通用任务式 API | `POST /tasks` | `GET /tasks/{id}` | `GET /tasks/{id}/files` |
