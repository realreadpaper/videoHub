...
import { compileWireRequest, generationTypes, sealGeneratedVideoSet, selectWireModelForRequest, } from "@hypit/hypit/generation";
/** ★ 改这里：包名，必须与 package.json 的 name 一致 */
export const providerModule = { name: "@local/provider-selfhosted-video", version: "1" };
/**
 ...
 */
export const capability = { module: { name: "@hypit/seedance", version: "1" }, name: "seedance-2-mini" };
/**
...
function text(value, subject) {
    if (typeof value !== "string" || value.length === 0)
        throw new Error(`${subject} 必须是非空字符串`);
    return value;
}
...
export function createSelfHostedVideoProvider(options) {
    ...
    async function json(path, credentials, init = {}) {
    ...
    const endpoint = {
        async start(context) {
            ...
            const request = await compileWireRequest(mapping, authored, async (artifact) => {
                ...
                throw new Error("本 Provider 尚未配置参考素材托管：请实现 start() 里的 ★服务适配点 1，"
