...
# -*- coding: utf-8 -*-
"""
《老子不娶了》（姜文风格）批量生产驱动 —— H3 两阶段管线 (单镜 15s = 362 帧)
...
用法:
  ...
  python3 10_run_film.py --shots 5            # 只跑第 5 镜（草稿+精修）
...
SEED = 20260916
H3_LENGTH = 362  # 15.08 秒 @ 24fps (17n+5 网格)
...
def stage1_draft(shot, force=False):
    ...
    hdr("阶段1 草稿 · 镜 %02d · %s · 672x384 x %d帧(15s) x 4步" % (shot["no"], shot["framing"], H3_LENGTH))
    sets = [
        ("1.unet_name", UNET), ("2.lora_name", LORA), ("3.clip_name", CLIP),
        ("6.prompt", shot["prompt"]),
        ("6.width", 384), ("6.height", 672), ("6.length", H3_LENGTH), ("6.task_type", "T2VA"),
        ("9.noise_seed", SEED),
...
def main():
    ...
    hdr("良心面试 · 食堂红烧 | %d 镜 | 阶段=%s | seed=%d | 单镜=%d帧(15s)" %
        (len(sel), stage, SEED, H3_LENGTH))
