...
echo "▶ 检查 ComfyUI..."
ALIVE=$(ssh -o ConnectTimeout=20 $SRV "curl -s -m 6 -o /dev/null -w '%{http_code}' $API/system_stats 2>/dev/null" 2>/dev/null)
if [ "${ALIVE:-000}" != "200" ]; then
  ...
  for i in $(seq 1 18); do
    sleep 10
    A=$(ssh -o ConnectTimeout=15 $SRV "curl -s -m 5 -o /dev/null -w '%{http_code}' $API/system_stats 2>/dev/null" 2>/dev/null)
...
echo "▶ 强制清显存后启动流水线..."
ssh $SRV "curl -s -m 60 -X POST $API/free -H 'Content-Type: application/json' -d '{\"unload_models\":true,\"free_memory\":true}' >/dev/null 2>&1; sleep 15; cd $RMT && setsid nohup bash run_server.sh > run.log 2>&1 < /dev/null &"
sleep 5
